...
void run_filter_diff(std::uint64_t seed) {
  ...
  for (const std::int64_t n : kLengths) {
    ...
    for (const bool clustered : {false, true}) {
      for (const int pct : nightly() ? std::vector<int>(std::begin(sel_pcts_full),
                                                        std::end(sel_pcts_full))
                                     : std::vector<int>(std::begin(sel_pcts_pr),
                                                        std::end(sel_pcts_pr))) {
...
TEST(DiffFilter, AllBackendsMatchOracle) {
  for_each_backend([&](quiver::Isa isa) {
    for_each_element_type([&]<class T>(T) {
      run_filter_diff<T>(0xD1FF0002ull + static_cast<std::uint64_t>(isa));
    });
  });
}

  ...
  for (const std::int64_t n : kLengths) {
    ...
    for (int opi = 0; opi < 6; ++opi) {
      ...
      for (const std::uint8_t* vd : {static_cast<const std::uint8_t*>(nullptr),
        ...
        const std::int64_t want_count = ref::predicate_bitmap(n, want_bits.data(),
            [&](std::int64_t i) {
              return ref::valid(vd, i) && ref::compare_one(op, v[static_cast<std::size_t>(i)],
                                                           comparand);
            });
        std::vector<std::uint8_t> got_bits(static_cast<std::size_t>(bytes) + 1, 0xEE);
        const std::int64_t got_count = quiver::compare_bitmap(
            op, quiver::BatchView<T>{v.data(), n}, comparand, quiver::BitmapView{vd},
            got_bits.data());
        ...
        const std::int64_t sv_count = quiver::compare_selvec(
            op, quiver::BatchView<T>{v.data(), n}, comparand, quiver::BitmapView{vd},
            got_idx.data());
...
TEST(DiffCompare, AllBackendsMatchOracle) {
  for_each_backend([&](quiver::Isa isa) {
    for_each_element_type([&]<class T>(T) {
      run_compare_diff<T>(0xD1FF0001ull + static_cast<std::uint64_t>(isa));
    });
  });
}

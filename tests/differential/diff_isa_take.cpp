...
void run_take_diff(std::uint64_t seed) {
  ...
  for (const std::int64_t n : kLengths) {
    ...
    quiver::take(quiver::BatchView<T>{dict.data(), kDict}, quiver::SelVec{idx.data(), n},
                 got.data());
    for (std::int64_t i = 0; i < n; ++i) {
      ASSERT_EQ(std::memcmp(&got[static_cast<std::size_t>(i)],
                            &dict[idx[static_cast<std::size_t>(i)]], sizeof(T)),
                0)
          << i;
    }
...
TEST(DiffTake, AllBackendsMatchOracle) {
  for_each_backend([&](quiver::Isa isa) {
    for_each_element_type(
        [&]<class T>(T) { run_take_diff<T>(0xD1FF0005ull + static_cast<std::uint64_t>(isa)); });
  });
}

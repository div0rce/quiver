...
- **Alternatives:** (1) `std::span`-based signatures — rejected: `size_t` vs `int64` friction, no place to hang bitmap/selvec semantics, harder future C shim; (2) rich owning/RAII types — rejected: violates T6/REQ-API-003; (3) pure C-style pointer+length everywhere — rejected: charter names view structs; (4) **first-party trivially-copyable view structs + thin constrained-template façade over concrete per-type `detail::` symbols** (selected). *0.8 amendment (ADR-027):* `std::span`/contiguous-range forms exist as a zero-cost convenience overlay that forwards to these primitives; the primitives themselves remain span-free, preserving the C-shim path this ADR reserved.

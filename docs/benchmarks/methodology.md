...
- **Fixed buffer placement** (REQ-BENCH-013 fixed-layout school): in the noise-gate-sensitive families — currently `arith_guarded` and `hash`, adopted per family as margins demand — operand and result buffers allocate at named 4 KiB phases from a huge-page-backed arena (`bench/harness/phase.h`), making the load↔store memory-pipeline interaction part of the method instead of allocator behavior. Unpinned placement left operand loads 4 KiB-aliased against the result store stream (a false store→load dependence on ~95% of loads, measured on intel-coffee-lake), whose per-process-variable stall cost pushed short-SIMD-loop CVs past the §4 screen. Other families still allocate plainly. Placement is an in-process measurement detail: axes, statistics, and orchestration are unchanged, so QLM stays at 1 (REQ-LEDGER-014).
...
≥10 process-level repetitions, each a **fresh process**, run order shuffled per repetition with a recorded seed (REQ-LEDGER-006 interleaving defense); median + min with seeded percentile-bootstrap 95% CIs (B=10,000, ADR-020, the entry's CI pair reports the **median** estimator; the min is a point estimate); CV screening (>5% excluded until rerun, 3-5% published with the `noisy` flag and a note — `--rerun-noisy N` performs that rerun in the same invocation: up to N further full fresh-process repetition sets for the rejected entries only, each **replacing** the prior measurement rather than merging with it, with every attempt's raw files kept side by side (`rerun<P>-rep<K>--…`) and the surviving entry's notes recording the full CV chain); environment manifests per run with an empty-`deviations` requirement for publishability (REQ-LEDGER-013); append-only results under `ledger/results/` (ADR-021). Registered machines only, never CI runners.

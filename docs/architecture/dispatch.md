...
This module is how one function call ends up running the right CPU-specific code.
The first time you call an operation, Quiver detects the CPU, picks the fastest backend it supports,
remembers that choice, and calls it. Every later call takes the fast path with almost no overhead.
You can override the choice (for benchmarking) or pin it at build time, but by default it just works.

```mermaid
flowchart TD
  C["Call an operation"] --> Q{"Already resolved<br/>for the current policy?"}
  Q -->|yes| H["Use the cached backend pointer"]
  Q -->|no| R["Resolve: pick the highest backend<br/>the policy allows, then cache it"]
  R --> H
  H --> RUN["Run the backend"]
  POL["Policy = CPU features,<br/>capped by QUIVER_ISA env,<br/>capped by any override"] -.-> R
```

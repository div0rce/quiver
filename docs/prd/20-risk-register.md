...
| R-19 | **RESOLVED (v0.7.0).** The nightly suite had never fully passed: the MSan-instrumented-libc++ leg (added M3) failed at Quiver's `project()` compiler check on every nightly since 2026-07-03, so `release.yml` withheld `publish` and the v0.6.0 tag has no artifacts (that hold stands; its tag predates the fix). Root cause: the job's `CXXFLAGS`/`LDFLAGS` link the MSan-instrumented `libc++abi.so` during CMake's compiler probe, but `-fsanitize=memory` was only added per-target after `project()`, leaving the probe without the MSan runtime (undefined `__msan_*`). Fix: the flag moved into the job's global env (#32), validated by a branch `workflow_dispatch` (first-ever green MSan leg), then the v0.7.0 tag ran the nightly green end to end and `release.yml` published the draft artifacts — the publish path is live-verified | — | — | closed; the release flow is the regression guard (every tag re-runs the nightly incl. MSan) | v0.7.0 release run; nightly job history |

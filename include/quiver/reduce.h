...
T reduce_min(BatchView<T> in, BitmapView validity, SelVec sel) noexcept {
  ...
  return detail::k6_reduce_min(in.data, in.len, validity.bits, detail::nonnull_sel(sel.idx),
                               sel.len);
}
...
T reduce_max(BatchView<T> in, BitmapView validity, SelVec sel) noexcept {
  ...
  return detail::k6_reduce_max(in.data, in.len, validity.bits, detail::nonnull_sel(sel.idx),
                               sel.len);
}
...
SumType<T> reduce_sum_wrap(BatchView<T> in, BitmapView validity, SelVec sel) noexcept {
  ...
  return detail::k6_reduce_sum_wrap(in.data, in.len, validity.bits, detail::nonnull_sel(sel.idx),
                                    sel.len);
}
...
bool reduce_sum_checked(BatchView<T> in, BitmapView validity, SelVec sel,
  ...
  return detail::k6_reduce_sum_checked(in.data, in.len, validity.bits, detail::nonnull_sel(sel.idx),
                                       sel.len, out_sum);
}
